...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		if(chartEditorSave.data.chartEditorThemeMusic != null) welcomeMusic.active = chartEditorSave.data.chartEditorThemeMusic;
		...
		{
			var note:StrumNote = new StrumNote(startX + (GRID_SIZE * i), startY, i % GRID_COLUMNS_PER_PLAYER, 0, true);
		...
		if(Conductor.songPosition != lastTime || forceDataUpdate)
		{
			...
			for (note in curRenderedNotes)
			{
				...
				if(Conductor.songPosition > note.strumTime && lastTime <= note.strumTime)
				{
					...
					if (note.mustPress && lilBuddiesEnabled)
					{
	...
	var lilBuddiesBox:PsychUIButton;
	function addViewTab()
	{
		...
		btnY++;
		btnY += 20;
		if (chartEditorSave.data.lilBuddies == null)
			chartEditorSave.data.lilBuddies = false;
		lilBuddiesBox = new PsychUIButton(btnX, btnY, lilBuddiesEnabled ? "  Lil' Buddies ON" : "  Lil' Buddies OFF", function()
		{
			lilBuddiesEnabled = !lilBuddiesEnabled;
			chartEditorSave.data.lilBuddies = lilBuddiesEnabled;
			lilBf.visible = lilOpp.visible = lilStage.visible = lilBuddiesEnabled;

			lilBuddiesBox.text.text = lilBuddiesEnabled ? "  Lil' Buddies ON" : "  Lil' Buddies OFF";
		}, btnWid);
		lilBuddiesBox.text.alignment = LEFT;
