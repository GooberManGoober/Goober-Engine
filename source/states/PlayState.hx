...
class PlayState extends MusicBeatState
{
	...
	public var grpSustainSplashes:FlxTypedGroup<SustainSplash> = new FlxTypedGroup<SustainSplash>();
	...
	override public function create()
	{
		...
		subtitleText = new FlxText(200, FlxG.height * 0.7, FlxG.width - 400, "");
		subtitleText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		healthLerp = FlxMath.lerp(healthLerp, health, .2 / (ClientPrefs.data.framerate / 60));
		...
		switch(eventName) {
			...
			case "Subtitle Event":
				...
				subtitleEvent(
					...
					triggerInfo[3], 
					Std.parseInt(triggerInfo[4]), 
					Std.parseFloat(triggerInfo[5])
				);
	...
	public function subtitleEvent(text:String = "", r:Int = 255, g:Int = 255, b:Int = 255, fontName:String = "vcr.ttf", size:Int = 32, width:Float = 0)
	{
		...
		subtitleText.color = FlxColor.fromRGB(r, g, b);
		subtitleText.setFormat(Paths.font(fontName), size, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
	...
	public function focusCamera(target:String = 'boyfriend', ?X:Float = 0, ?Y:Float = 0, ?Time:Float = 1, ?ease:String = 'linear', hasTween:Bool = false)
	{
		switch(target.toLowerCase().trim())
		{
			case 'bf' | 'boyfriend' | 'player':
				...
				positionData.x -= boyfriend.cameraPosition[0] - boyfriendCameraOffset[0] + X;
				positionData.y += boyfriend.cameraPosition[1] + boyfriendCameraOffset[1] - Y;

			case 'gf' | 'girlfriend':
				positionData.set(gf.getMidpoint().x, gf.getMidpoint().y);
				positionData.x += gf.cameraPosition[0] + girlfriendCameraOffset[0] + X;
				positionData.y += gf.cameraPosition[1] + girlfriendCameraOffset[1] - Y;

			case 'dad' | 'opponent':
				positionData.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
				positionData.x += dad.cameraPosition[0] + opponentCameraOffset[0] + X;
				positionData.y += dad.cameraPosition[1] + opponentCameraOffset[1] - Y;
	...
	*/
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(uiFolder + 'num' + Std.int(i) + uiPostfix));
